...
export function handleCampaignCreated(event: CampaignCreatedEvent): void {
  ...
  // Bind to the smart contract to fetch details not present in the event (like deadline)
...
export function handleRefundClaimed(event: RefundClaimedEvent): void {
  let campaign = Campaign.load(event.params.id.toString())
  if (campaign) {
    // Subtract the refunded amount from the total ETH raised tracking
    campaign.ethRaised = campaign.ethRaised.minus(event.params.amountEth)
    // If refunds are being claimed, the campaign has failed its target/deadline requirements
