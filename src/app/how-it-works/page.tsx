...
export default function HowItWorksPage() {
  return (
    <div className="flex flex-col min-h-screen bg-transparent text-foreground selection:bg-primary/30">
      {/* Hero Header */}
      <section className="w-full pt-4 md:pt-10 px-6 md:px-4 overflow-hidden relative">
        <div className="max-w-4xl mx-auto text-center relative z-10 flex flex-col gap-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-[9px] md:text-xs font-black uppercase tracking-widest mx-auto">
      ...
      <section className="w-full py-4 md:py-6 px-6 md:px-4 border-b">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <div className="p-4 md:p-8 rounded-2xl md:rounded-[2rem] bg-foreground text-white flex flex-col gap-3 relative overflow-hidden group">
          ...
          <div className="p-4 md:p-8 rounded-2xl md:rounded-[2rem] bg-white border border-border shadow-sm flex flex-col gap-3 relative overflow-hidden group">
      ...
      {/* Detailed Workflow */}
      <section className="w-full py-6 md:py-10 px-6 md:px-4">
          ...
          <div className="flex flex-col gap-6 md:gap-10">
            {STEPS.map((step, index) => (
              <div 
                key={step.number} 
                className={cn(
                  "flex flex-col md:flex-row gap-4 md:gap-8 items-start group",
                  index % 2 !== 0 && "md:flex-row-reverse"
                )}
              >
                <div className="flex-1 space-y-3">
                  <div className="flex items-center gap-3">
                    <span className="text-3xl md:text-5xl font-black text-primary/10 group-hover:text-primary/20 transition-colors">
                      {step.number}
                    </span>
                    <div className={cn("p-2.5 md:p-3 rounded-xl md:rounded-2xl border flex items-center justify-center", step.color)}>
                  ...
                  <div className="space-y-1.5 md:space-y-2">
                    ...
                    <div className="prose prose-sm md:prose-base max-w-none text-muted-foreground leading-relaxed">
                      ...
                      {(step.security || step.tech) && (
                        <div className="mt-2 p-3 bg-muted/50 rounded-xl md:rounded-[1.5rem] border-l-4 border-primary italic text-[10px] md:text-xs">
                          {step.security || step.tech}
                        </div>
                      )}
                    </div>
                ...
                <div className="hidden md:flex flex-1 items-center justify-center">
                   <div className="w-full max-w-[240px] aspect-square rounded-[2rem] bg-primary/5 border border-primary/10 flex items-center justify-center relative overflow-hidden group">
                      <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                      <step.icon className="h-16 w-16 text-primary/20 group-hover:scale-110 group-hover:text-primary transition-all duration-700" />
                   </div>
                </div>
              </div>
            ))}
